...
def create_base_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parent parser for tape functions", add_help=False
    )
    parser.add_argument("model_type", help="Base model class to run")
    parser.add_argument(
        "--model_config_file",
        default=None,
        type=utils.check_is_file,
        help="Config file for model",
    )
    parser.add_argument(
        "--vocab_file", default=None, help="Pretrained tokenizer vocab file"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Local rank of process in distributed training. " "Set by launch script.",
    )
    parser.add_argument(
        "--tokenizer",
        choices=["iupac", "unirep"],
        default="iupac",
...
def create_train_parser(
    ...
    parser = argparse.ArgumentParser(
        description="Run Training on the TAPE datasets", parents=[base_parser]
    )
    parser.add_argument(
        "task",
        choices=list(registry.task_name_mapping.keys()),
    ...
    parser.add_argument(
        "--data_dir",
        default="./data",
        type=utils.check_is_dir,
        help="Directory from which to load task data",
    )
    parser.add_argument(
        "--data_file_prefix",
        type=str,
        help="The prefix of task LMDB file such as pfam/pfam.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fp16", action="store_true", help="Whether to use fp16 weights"
    )
    parser.add_argument(
        "--warmup_steps",
        default=10000,
    ...
    parser.add_argument(
        "--exp_name", default=None, type=str, help="Name to give to this experiment"
    )
    parser.add_argument(
        "--from_pretrained",
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="A file name for pretrained model weights."
        "If this is not set and --from_pretrained is on, 'pytorch_model.bin' is used.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Frequency of eval pass. A value <= 0 means the eval pass is " "not run",
    )
    parser.add_argument(
        "--save_freq",
        default=1,
        type=utils.int_or_str,
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="How many epochs without improvement to wait before ending " "training",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Name of a checkpoint file such as checkpoint.bin or checkpoint-1234.bin",
    )
    parser.add_argument(
        "--keep_latest",
        action="store_true",
        help="Keep the latest checkpoint and model file only",
    )
    parser.add_argument(
        "--val_metric", type=str, choices=list(registry.metric_name_mapping.keys())
    )
    return parser
...
def create_eval_parser(base_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Eval on the TAPE Datasets", parents=[base_parser]
    )
    parser.add_argument(
        "task",
        choices=list(registry.task_name_mapping.keys()),
    ...
    parser.add_argument(
        "--split", default="test", type=str, help="Which split to run on"
    )
    return parser


def create_embed_parser(
    base_parser: argparse.ArgumentParser,
) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
    ...
    parser.add_argument(
        "data_file", type=str, help="File containing set of proteins to embed"
    )
    ...
    parser.add_argument(
        "--output_hidden",
        action="store_true",
        help="It true, saves all hidden states at every amino acid "
        "position in the sequence.",
    )
    parser.add_argument(
        "--avg_hidden_layers",
        type=str,
        help="Comma-separated integers indicating which hidden layers to use "
        "to calculate averaged embeddings for each token, such as 11,12. "
        "0 for token embedding."
    )
...
def run_train(args: typing.Optional[argparse.Namespace] = None, env=None) -> None:
    ...
    if (
        args.task
        not in ["masked_language_modeling", "language_modeling"]
    ) and (args.data_file_prefix is not None):
        raise RuntimeError(
            "Currently args.data_file_prefix can only be specified with "
            "masked_language_modeling or language_modeling tasks."
        )

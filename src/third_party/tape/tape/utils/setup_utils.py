...
def setup_logging(
    ...
    log_level: typing.Union[str, int] = None,
) -> None:
...
def setup_optimizer(model, learning_rate: float):
...
def setup_dataset(
    ...
    return task_spec.dataset(
        data_dir, split, tokenizer, data_file_prefix=data_file_prefix
    )  # type: ignore
...
def setup_loader(
    ...
    batch_size = (
        get_effective_batch_size(batch_size, local_rank, n_gpu, gradient_accumulation_steps)
        * n_gpu
    )
    # WARNING: this will fail if the primary sequence is not the first thing the dataset returns
    batch_sampler = BucketBatchSampler(sampler, batch_size, False, lambda x: len(x[0]), dataset)

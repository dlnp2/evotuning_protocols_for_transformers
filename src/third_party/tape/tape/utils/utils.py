...
def get_expname(
    ...
    if exp_name is None:
        ...
        exp_name = (
            f"{task}_{model_type}_{time_stamp}_{random.randint(0, int(1e6)):0>6d}"
        )
    return exp_name
...
def get_effective_batch_size(
    batch_size: int, local_rank: int, n_gpu: int, gradient_accumulation_steps: int = 1
) -> int:
...
def get_num_train_optimization_steps(
    dataset: Dataset, batch_size: int, num_train_epochs: int
) -> int:
...
class MetricsAccumulator:
    ...
    def step(self) -> typing.Dict[str, float]:
        ...
        metricstmp = {
            name: value / self._nacc_steps for name, value in self._metricstmp.items()
        }
    ...
    def final_metrics(self) -> typing.Dict[str, float]:
        return {
            name: value / self._nupdates for name, value in self._totalmetrics.items()
        }
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if exc_args and "CUDA out of memory" in exc_args[0]:
            ...
            if self._gradient_accumulation_steps is not None:
                ...
                message = (
                    f"CUDA out of memory. Reduce batch size or increase "
                ...
                message = (
                    f"CUDA out of memory. Reduce batch size to fit each "
...
def check_avg_hidden_layers(avg_hidden_layers: str, n_layers: int) -> typing.List[int]:
    layers = [int(l) for l in avg_hidden_layers.split(",")]
    if not all([l in range(n_layers + 1) for l in layers]):
        raise ValueError(
            f"Each layer must be in [0, {n_layers}], but given {avg_hidden_layers}"
        )

...
class ProteinConfig(object):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if pretrained_model_name_or_path in cls.pretrained_config_archive_map:
            ...
            config_file = str(pretrained_model_name_or_path)
        else:
...
class ProteinModel(nn.Module):
    ...
    def save_pretrained(
        self,
        save_directory,
        epoch_id: typing.Optional[int] = None,
        ...
        if epoch_id is not None:
            if keep_latest:
                for binfile in glob.glob(
                    os.path.join(save_directory, "pytorch_model-*.bin")
                ):
                    os.remove(binfile)
            output_model_file = os.path.join(
                save_directory, f"pytorch_model-{epoch_id}.bin"
            )
            torch.save(model_to_save.state_dict(), output_model_file)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        pretrained_weight_file_name = kwargs.pop("pretrained_weight_file_name", None)
        ...
        elif type(config) in [str, PosixPath]:  # path of a config json file
            config, model_kwargs = cls.config_class.from_pretrained(
                config,
                *model_args,
                cache_dir=cache_dir,
        ...
        # Load model
        if pretrained_weight_file_name is not None:
            pretrained_model_name_or_path = os.path.join(
                pretrained_model_name_or_path, pretrained_weight_file_name
            )

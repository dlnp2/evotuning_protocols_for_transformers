...
from .modeling_utils import VariantEffectPredictionHead
...
BERT_PRETRAINED_CONFIG_ARCHIVE_MAP = {"bert-base": URL_PREFIX + "bert-base-config.json"}
    ...
    def forward(self, hidden_states, attention_mask, chunks=None):
        ...
        if chunks is not None:
            ...
            for start in range(0, len(self.layer), chunk_size):
                outputs = checkpoint(
                    self.run_function(start, chunk_size), hidden_states, attention_mask
                )
                if self.output_hidden_states:
...
class ProteinBertModel(ProteinBertAbstractModel):
    ...
    def forward(self, input_ids, input_mask=None):
        ...
        encoder_outputs = self.encoder(
            embedding_output, extended_attention_mask, chunks=None
        )
...
class ProteinBertForMaskedLM(ProteinBertAbstractModel):
    ...
    def tie_weights(self):
        ...
        self._tie_or_clone_weights(
            self.mlm.decoder, self.bert.embeddings.word_embeddings
        )
...
class ProteinBertForContactPrediction(ProteinBertAbstractModel):
    def __init__(self, config):
        ...
        self.predict = PairwiseContactPredictionHead(
            config.hidden_size, ignore_index=-1
        )
